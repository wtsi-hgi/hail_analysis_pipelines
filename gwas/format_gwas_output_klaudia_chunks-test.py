...
outpath="/lustre/scratch119/realdata/mdt2/projects/interval_wgs/analysis/hail_analysis/gwas/nmr_results/tables"
    ...
    #df= pd.read_csv(tsv1, delimiter="\t", compression='gzip')
       ...
       # chunk[['nmr_phenotypes']]=chunk[['nmr_phenotypes']].apply(lambda x: x.str.strip("\"\""), axis=1)
        ...
        for i, x in chunk.groupby('nmr_phenotypes'):
            ...
            x.to_csv(p, sep="\t", header=(not os.path.exists(tsvout)),compression='gzip',index=False, mode='a')
