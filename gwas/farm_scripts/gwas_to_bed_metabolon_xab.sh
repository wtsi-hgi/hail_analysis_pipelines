...
CHUNK=/lustre/scratch119/realdata/mdt2/projects/interval_wgs/analysis/hail_analysis/gwas/final_results/gwas_tables/metabolon/xab
...
while read -r FILE; do
    ...
    zless "${FILE}"  |tr ':' '\t' > "${outdir}"/interimxab.bed
	#s3cmd put "${FILE}" "${BUCKET}/${CHROMOSOME}-vcf/${gs_name}"
    awk 'BEGIN{FS=OFS="\t"} {$2 = $2 OFS $2} 1' ${outdir}/interimxab.bed| tail -n+2| gzip -c > ${outdir}/${fbname}.bed.gz
    rm ${outdir}/interimxab.bed
	echo ${COUNT}
done < "${CHUNK}"
