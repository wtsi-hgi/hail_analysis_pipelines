...
CHUNK=/lustre/scratch119/realdata/mdt2/projects/interval_wgs/analysis/hail_analysis/gwas/final_results/gwas_tables/metabolon/xaa
...
while read -r FILE; do
    ...
    zless "${FILE}"  |tr ':' '\t' > "${outdir}"/interimxaa.bed
	#s3cmd put "${FILE}" "${BUCKET}/${CHROMOSOME}-vcf/${gs_name}"
    awk 'BEGIN{FS=OFS="\t"} {$2 = $2 OFS $2} 1' ${outdir}/interimxaa.bed| tail -n+2| gzip -c > ${outdir}/${fbname}.bed.gz
    rm ${outdir}/interimxaa.bed
	echo ${COUNT}
done < "${CHUNK}"
